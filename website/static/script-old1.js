$(document).ready(function() {  
	...
	return 'M' + (d.y - 8) + ',' + (d.x - 0) +
	  "C" + (d.y + d.parent.y + 120) / 2 + "," + (d.x - 0) +
	  " " + (d.y + d.parent.y + 120) / 2 + "," + (d.parent.x + 0) +
	  " " + (d.parent.y + 8) + "," + (d.parent.x + 0);
	/*return 'M' + (d.y - 8) + ',' + (d.x - 0) +
	  "C" + (d.y + d.parent.y + (d.parent.textwidth)) / 2 + "," + (d.x - 0) +
	  " " + (d.y + d.parent.y + (d.parent.textwidth)) / 2 + "," + (d.parent.x + 0) +
	  " " + (d.parent.y + d.parent.textwidth + 8) + "," + (d.parent.x + 0);*/
	};   
  ...
  function getTreeData(json, transferTextWidth=false) {
    ...
    /*if(transferTextWidth) {
        root.each(function(d) { d.y = d.depth * 180; d.x0 = d.x; d.y0 = d.y; d.textwidth0 = d.textwidth; console.log(d); });
    } else {
        root.each(function(d) { d.y = d.depth * 180; d.x0 = d.x; d.y0 = d.y; });
    }*/
    root.each(function(d) { d.y = d.depth * 120; d.x0 = d.x; d.y0 = d.y; });
    return root;
  }
  ...
  function drawElements(node) {
    ...
    textGroup.append("text")
      ...
      .each(function(d) {
        ...
        d.textwidth0 = d.textwidth
      });

    // Add circles above each node
    node.append("circle")
      .attr("r", 2)
      //.attr("transform", function(d) { return "translate(-18,0)"; })
      //.attr("transform", function(d) { if(d.textwidth) return "translate(" - ((d.textwidth/2) - 8) + ",0)"; })
    ...
    node.append("circle")
      .attr("r", 5)
      //.attr("transform", function(d) { return "translate(16, 0)"; })
      .attr("transform", function(d) { return "translate(" + ((d.textwidth) + 10) + ",-2)"; })
    ...
    textGroup
      ...
      .style("fill-opacity", function(d) {
          if(d.children || d.data.data.rank == max_rank) { return 0.8; }
          else { return 0.6; }
        }
      )
      ...
      .attr("transform", function(d) {
        if(d.data.data.rank == 9) {
          return "translate(-5,-" +  ((d.textheight + 30) / 2) + ")";
        }
        return "translate(-5,-" +  ((d.textheight + 15) / 2) + ")";
      })
      .attr('rx', 10)
      .attr('ry', 10);
  }
  ...
  function updateTree(source, shallowestDepth = 0) {
    ...
    var linkEnter = link.enter()
      ...
      .attr("d",  function (d) {
        //console.log(d);
        var o = {x: d.parent.x0, y: d.parent.y0, parent: {x: d.parent.x0, y: d.parent.y0, textwidth: d.textwidth0}, textwidth: d.textwidth0};
        return connector(o);
      });
    ...
    var linkMerge = link.merge(linkEnter).transition()
      ...
      .attr('d', function(d) {
        var o = d
        //console.log(d)
        //d.y = d.y - (d.textwidth/2) - 5
        //d.y0 = d.y0 - (d.textwidth/2) - 5
  ...
  function click(d) {
    ...
    if (!d.children && !d._children) {
      ...
      d3.json(jsonPath, function(error, json) {
        ...
        root = getTreeData(new_lineage, transferTextWidth=true);
